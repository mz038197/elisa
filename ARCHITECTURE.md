...
- **WebSocket heartbeat + send pacing**: Server sends protocol-level ping frames every 30s via `ws.ping()`. Browsers auto-respond with pong. Connections missing a pong cycle are terminated. `sendEvent()` yields to the event loop (via `setImmediate`) after each `ws.send()` to prevent burst-flooding the Vite dev proxy during rapid-fire event loops.
