...
- **WebSocket heartbeat**: Server sends protocol-level ping frames every 30s via `ws.ping()`. Browsers auto-respond with pong. Connections missing a pong cycle are terminated. Prevents proxy idle-timeout disconnects during heavy agent work.
