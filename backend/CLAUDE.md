...
- **WebSocket heartbeat + send pacing**: Server sends protocol-level pings every 30s (`WS_PING_INTERVAL_MS`). Connections that miss a pong are terminated via `ws.terminate()`. `sendEvent()` yields to the event loop after each `ws.send()` (via `setImmediate`) to prevent burst-flooding the Vite proxy when events fire in tight loops (e.g., per-task TDD test results). `wsAlive` WeakMap tracks per-connection liveness.
