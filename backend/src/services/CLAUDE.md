...
- **executePhase.ts** -- Streaming-parallel task execution (Promise.race pool, up to 3 concurrent), workspace setup (cleans stale `.elisa/` artifacts on re-builds), git mutex, context chain, token budget enforcement. Emits `meeting_blocking_task`/`meeting_unblocking_task` events when tasks enter/leave meeting-blocked state in `waitForMeetings()`.
...
Claude-powered agent responses for meeting conversations. Uses Anthropic SDK (Haiku model via `NARRATOR_MODEL_DEFAULT` constant). Dual-call architecture: `generateResponse()` makes two parallel API calls via `Promise.allSettled` -- a chat call (persona + build context + "NEVER output JSON", `MEETING_CHAT_MAX_TOKENS=300`) and a canvas call (canvas schema + "Output ONLY a valid JSON object", `MEETING_CANVAS_MAX_TOKENS=4096`). Canvas call skipped when no `CANVAS_INSTRUCTIONS` entry exists for the meeting's `canvasType`. Optional `options` parameter: `focusContext` (scopes prompts to a specific task) and `previousDesigns` (lists already-designed elements for consistency). When provided, chat prompt gets "Your Focus" + "Already Designed" sections; canvas prompt gets "Scope" + "Previously Designed" sections. `parseCanvasResponse()` strips accidental fencing then tries `JSON.parse` with `sanitizeJsonStrings` fallback for literal newlines in draw code. Each call fails gracefully: chat failure returns fallback text, canvas failure returns `undefined`. Converts `MeetingMessage[]` to Claude format (kid->user, agent->assistant). 45s timeout via `withTimeout()` (`MEETING_AGENT_TIMEOUT_MS`), canvas timeout logged as `console.warn` (expected under load), chat timeout logged as `console.error`.

### meetingMaterializer.ts (meeting artifact generation)
Pure functions that materialize meeting canvas data into real files in the nugget workspace. Dispatches by canvas type with organized subdirectories: `explain-it` -> `docs/{title}.md`, `launch-pad` -> `web/launch-page.html` (4 template variants), `campaign` -> `marketing/poster.html` + `marketing/social-card.html`, `interface-designer` -> `design/interfaces.json`, `theme-picker` -> `design/theme-config.json`, `design-preview` -> `design/{scene}-design.json` (includes optional `color` and `draw` fields per element). Blueprint and bug-detective produce no files. Top-level `materialize(canvasType, data, nuggetDir)` writes files and returns `{ files, primaryFile }`.

### meetingTriggerWiring.ts (meeting trigger wiring)
Wraps `MeetingTriggerEngine` + `MeetingService` for the orchestrator. Team filtering via `setSpec(sessionId, spec)`: always-on defaults (`buddy-agent`, `doc-agent`, `architecture-agent`, `debug-convergence`, `design-task-agent`) fire without team blocks; opt-in agents require `meeting_team` entries in the spec. `evaluateAndInvite()` and `evaluateAndInviteForTask()` both apply `isEnabledForSession()` check before creating invites. Deduplicates per meeting type per session. `clearSession()` removes dedup state and spec. Wired into orchestrator at `plan_ready`, `task_completed` (fire-and-forget via ExecutePhase), `deploy_started`, `session_complete`, and pre-task events.

### taskMeetingTypes.ts (task-level meeting types)
Registers meeting types with `event: 'task_starting'` and keyword-based filter functions. Exports `DESIGN_KEYWORDS` regex (sprite, art, icon, theme, etc.) and `SCAFFOLD_SKIP_KEYWORDS` regex (scaffold, setup, test, lint, etc.) for reuse in `executePhase.ts`. Scaffold skip takes priority: tasks matching both skip and design keywords do NOT trigger meetings. Currently registers `design-task-agent` (Design Review). Uses `design-preview` canvas for live visual design collaboration. `registerTaskMeetingTypes(registry)` called at startup.
...
### buddyAgentMeeting.ts (buddy agent meeting type)
Registers the `buddy-agent` meeting type with `canvasType: 'explain-it'`. Triggers on `task_completed` when >= 25% done (mid-build check-in). Agent persona is "Buddy" -- friendly teammate who explains progress and takes feedback. Always-on default (fires without team blocks). `registerBuddyAgentMeeting(registry)` called at startup.
